...
<template>
  <view class="uni-content">
    ...
    <uni-list class="mt10">
      <uni-list-item class="item" title="ConnectCode" :rightText="connectCode || '未生成'" link>
      </uni-list-item>
      <uni-list-item class="item" @click="generateConnectCode" title="生成/重置 ConnectCode" link>
      </uni-list-item>
      <uni-list-item class="item" @click="copyConnectCode" v-if="connectCode" title="复制 ConnectCode" link>
      </uni-list-item>
    </uni-list>
    <!-- #ifndef MP -->
    <uni-list class="mt10">
...
export default {
  ...
  data() {
    return {
      ...
      connectCode: '', // 存储用户的 connectCode
      connectCodeLoading: false // 控制加载状态
    }
  },
  async onShow() {
  ...
  methods: {
    // 获取用户的 connectCode
    async fetchConnectCode() {
      if (!this.userInfo._id) return
      
      try {
        uni.showLoading({ title: '加载中' })
        // 显式传递用户ID
        const res = await cloudObject.getConnectCode(this.userInfo._id)
        if (res.errCode === 0) {
          this.connectCode = res.data.connectCode
        } else if (res.errCode === 404) {
          // 用户还没有生成 connectCode
          this.connectCode = ''
        } else {
          uni.showToast({
            title: res.errMsg || '获取 ConnectCode 失败',
            icon: 'none'
          })
        }
      } catch (e) {
        uni.showToast({
          title: '获取 ConnectCode 失败',
          icon: 'none'
        })
        console.error(e)
      } finally {
    ...
    async generateConnectCode() {
      if (this.connectCodeLoading) return
      this.connectCodeLoading = true
      
      try {
        uni.showLoading({ title: '生成中' })
        // 显式传递用户ID
        const res = await cloudObject.generateConnectCode(this.userInfo._id)
        if (res.errCode === 0) {
          this.connectCode = res.data.connectCode
          uni.showToast({
            title: '生成成功',
